...
@router.patch("/faq/{faq_id}/publish")
async def publish_faq(faq_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    return await publish_general_faq(faq_id, db)

@router.patch("/faq/{faq_id}/draft")
async def unpublish_faq(faq_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    return await unpublish_general_faq(faq_id, db)
